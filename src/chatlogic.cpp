                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            edge->SetChildNode((*childNode).get()); //return raw pointer of unique ptr node
                            edge->SetParentNode((*parentNode).get());
                            _edges.push_back(edge.get()); //return raw pointer to input raw pointer edge

                            // find all keywords for current node
                            AddAllTokensToElement("KEYWORD", tokens, *edge);

                            // store reference in child node and parent node
                            (*childNode)->AddEdgeToParentNode(edge.get()); //raw pointer for outcoming pointer( use raw pointer)
                            (*parentNode)->AddEdgeToChildNode(std::move(edge)); //resource to a incoming pointer (use unique pointer)
                        }
